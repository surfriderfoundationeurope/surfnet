...
def save_flow(flow_dir, frame_nb, flow):
    output_name = os.path.join(
        flow_dir, '{:03d}_{:03d}'.format(frame_nb, frame_nb+1))
...
def verbose(frame0, frame1,  heatmap0, heatmap1, flow01):
    ...
    h, w = flow01.shape[:-1]
#
    frame0 = Image.fromarray(cv2.cvtColor(
        cv2.resize(frame0, (w, h)), cv2.COLOR_BGR2RGB))
    frame1 = Image.fromarray(cv2.cvtColor(
        cv2.resize(frame1, (w, h)), cv2.COLOR_BGR2RGB))

    frame0_to_warp = ToTensor()(frame0).unsqueeze(0).to('cuda')
    flow01_to_warp = torch.tensor(flow01).unsqueeze(0).to('cuda')
    frame0_warped = spatial_transformer(
        frame0_to_warp, -flow01_to_warp, device='cuda')
    ax0.imshow(frame0)
...
def extract_heatmaps_for_video(filenames, heatmaps_dir_for_video, flows_dir_for_video, model, transform):
    ...
    def get_heatmap(frame): return _get_heatmap(frame, model, transform)
...
def extract(args):
    ...
    model = create_model_centernet(
        arch='dla_34', heads={'hm': 1, 'wh': 2}, head_conv=256)
    ...
    annotation_filename = os.path.join(
        args.dataset_dir, 'annotations', 'annotations.json')
    ...
    for video in tqdm(annotations['videos']):
        ...
        images = [image for image in annotations['images']
                  if image['video_id'] == video['id']]
        images = sorted(images, key=lambda image: image['frame_id'])
        filenames = [os.path.join(
            args.dataset_dir, 'data', image['file_name']) for image in images]

        old_shape, new_shape = extract_heatmaps_for_video(
            filenames, heatmaps_dir_for_video, flows_dir_for_video, model, transform)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Extracting heatmaps produced by base network')
