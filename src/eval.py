...
def prec_recall_for_thres(thres, thres_nb, gt, pred, radius):
    ...
    for gt_frame, detection_frame in zip(gt, detections):
        ...
        if len(position_positives):
            ...
            assigned_positives_for_detections = np.argmin(
            ...
            for positive in range(len(position_positives)):
                ...
                if len(assigned_detections):
                    ...
                    if similarities[closest_detection] > 1 - radius:
                        ...
                        distances_true_positives.append(
                            similarities[closest_detection]
                        )
...
def plot_pr_curve(
    ...
    plt.suptitle(
        f"F-score {best_f1:.2f} (Precision {best_precision:.2f}, Recall {best_recall:.2f}) at threshold {best_thres:.2f}"
    )
...
def compute_precision_recall_hungarian(
    ...
    precision_list, recall_list = np.array(precision_list), np.array(
        recall_list
    )
    ...
    distances_true_positives_list_best_position = (
        distances_true_positives_list[best_position]
    )
    distances_false_positives_list_best_position = (
        distances_false_positives_list[best_position]
    )
...
def compute_model_outputs(arch):
    ...
    for batch_images, batch_labels in tqdm(loader):
        ...
        batch_heatmaps = (
            torch.sigmoid(batch_output[-1]["hm"]).squeeze(dim=1).cpu()
        )
...
def main(args=None):
    for arch in ["res_18", "mobilenetv3small", "dla_34"]:
        ...
        compute_precision_recall_hungarian(
            ...
            all_heatmaps,
            enable_nms=True,
            output_filename=f"evaluation_{arch}",
        )
