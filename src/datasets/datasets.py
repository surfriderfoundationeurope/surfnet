...
class SingleVideoDataset(torch.utils.data.Dataset):
    def __init__(self, video_name, transforms=None):
        self.video = torchvision.io.read_video(video_name)[0].permute(
            0, 3, 1, 2
        )
        self.annotation = json.load(open(video_name.replace(".MP4", ".json")))
    ...
    def __getitem__(self, index):
        ...
        if self.transforms is not None:
            ...
            annotation = self._resize_annotation(
                annotation, old_frame_shape, new_frame_shape
            )
    ...
    def _resize_annotation(self, annotation, old_frame_shape, new_frame_shape):
        ...
        for object_nb in annotation.keys():
            ...
            [top_left_x, top_left_y, width, height] = annotation[object_nb][
                "bbox"
            ]
            [center_x, center_y] = annotation[object_nb]["center"]

            annotation[object_nb]["bbox"] = [
                int(top_left_x * ratio_x),
                int(top_left_y * ratio_y),
                int(width * ratio_x),
                int(height * ratio_y),
            ]
            annotation[object_nb]["center"] = [
                int(center_x * ratio_x),
                int(center_y * ratio_y),
            ]
...
class SurfnetDataset(torch.utils.data.Dataset):
    def __init__(self, annotations_dir, data_dir, split="val"):
        ...
        if self.split == "train":
            annotation_filename = os.path.join(
                annotations_dir, "annotations_train.json"
            )
        else:
            annotation_filename = os.path.join(
                annotations_dir, "annotations_val.json"
            )
    ...
    def init_ids(self):
        ...
        for video in self.annotations["videos"]:
            ...
            for image in sorted(
                images_from_video, key=lambda x: x["frame_id"]
            ):
                ...
                organised_annotations[video["id"]].append(
                    (image, annotations_for_image)
                )
    ...
    def get_training_item(self, location_id):
        ...
        heatmap_filename = os.path.join(
            self.heatmaps_dir,
            image0["file_name"].split("/")[0],
            f"{frame_id_0:03d}.pickle",
        )
        ...
        flow_filename = os.path.join(
            self.flows_dir,
            image0["file_name"].split("/")[0],
            f"{frame_id_0:03d}_{frame_id_1:03d}.npy",
        )
...
def plot_loader(video_loader):
    ...
    if video_loader.dataset.split == "train":
        ...
        for Z_0, Z_0_translated, Phi_0_tilde, Phi_1_tilde, d_01 in zip(
            Z_0s[:, 0, :, :],
            Z_0s_translated[:, 0, :, :],
            Phi_0_tildes[:, 0, :, :],
            Phi_1_tildes[:, 0, :, :],
