...
def compare_heatmaps_with_and_without_ae():
    ...
    with torch.no_grad():
        for batch_nb, (batch_images, _) in tqdm(enumerate(loader)):
            ...
            batch_output_with_ae = torch.sigmoid(batch_output_with_ae).squeeze(
                dim=1
            )[0]

            fig, (ax0, ax1) = plt.subplots(1, 2)
            ax0.imshow(
                batch_heatmaps_without_ae.cpu().numpy(),
                cmap="gray",
                vmin=0,
                vmax=1,
            )
            ax1.imshow(
                batch_output_with_ae.cpu().numpy(), cmap="gray", vmin=0, vmax=1
            )
            plt.show()
...
def train_ae():
    ...
    train_dataset = FeatureDataset(
        "data/images/image_features_train", device=device
    )
    val_dataset = FeatureDataset(
        "data/images/image_features_val", device=device
    )
