...
class AutoEncoder(pl.LightningModule):
    ...
    def __init__(self, model=None):
        ...
        if model is None: 
            ...
            self.ae = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet', in_channels=512, out_channels=1, init_features=32, pretrained=False)
            self.encoder = nn.Sequential(self.ae.encoder1, self.ae.pool1,
                                         self.ae.encoder2, self.ae.pool2,
                                         self.ae.encoder3, self.ae.pool3,
                                         self.ae.encoder4, self.ae.pool4,
                                         self.ae.bottleneck
                                        )
        else: 
...
def compare_heatmaps_with_and_without_ae():
    ...
    full_model = nn.Sequential(*list(model.children())[:-2], ae.ae, *list(model.children())[-2:]).to(device)
    full_model.eval()
    model.eval()
...
def train_ae():
    ...
    train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=64, pin_memory=True)
...
train_dataset = FeatureDataset('data/images/image_features_train', device=torch.device('cpu'))
