...
def draw_bbox(image, anns):
    ...
    for ann in anns:
        ...
        # color.append(c)
        cv2.rectangle(image, (bbox_x,bbox_y),(bbox_x+bbox_w+bbox_y+bbox_h), color=(255,0,0))
...
for imgId in permutation[:30]:
    ...
    draw_bbox(anns)
    # plt.savefig('img_{}'.format(imgId),format='pdf',bbox_inches='tight')
    # plt.show()
