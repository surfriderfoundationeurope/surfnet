...
def annotations_cat(df_bboxes):
    ...
    cat_percentages = round(
        (df_bboxes["id_ref_trash_type_fk"].value_counts() / len(df_bboxes) * 100), 2
    ).to_frame()
    ...
    plt.legend(
        handles, categories[1:11], bbox_to_anchor=(1.02, 0.75), title="Categories"
    )
...
def img_view(df_images):
    ...
    plot = dfg.plot(
        kind="bar", title="Size by View", ylabel="Size", xlabel="View", figsize=(6, 5)
    )
