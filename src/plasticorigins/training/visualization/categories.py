...
def annotations_cat(df_bboxes):
    ...
    cat_percentages = round(
        (
            df_bboxes["id_ref_trash_type_fk"].value_counts()
            / len(df_bboxes)
            * 100
        ),
        2,
    ...
    table = pd.DataFrame(
        categories
    )  # creation of data frame of the categories
    ...
    table.rename(
        columns={0: "Category"}, inplace=True
    )  # rename the category column
    new_table = table.set_index("ID").drop(
        [0]
    )  # final df, dropping excess info
...
def img_context(df_images):
    ...
    table.rename(
        columns={0: "Context", "context": "Percentages"}, inplace=True
    )
...
def img_quality(df_images):
    ...
    table.rename(
        columns={0: "Quality", "image_quality": "Percentages"}, inplace=True
    )
