...
import yolov5
from yolov5.utils.general import non_max_suppression, make_divisible
...
class DetectTorchScript(nn.Module):
    ...
    def __init__(self, weights, device=None, conf=0.35, iou=0.50, class_names=None, dnn=False, data=None):
        super().__init__()
    ...
    def forward(self, im):
        ...
        x = np.ascontiguousarray(im[:,:,:,::-1].transpose((0, 3, 1, 2)))  # BHWC to BCHW, and BGR to RGB
    ...
    def detect(self, ims):
        ...
        pred2 = non_max_suppression(pred, conf_thres=self.conf_thres, iou_thres=self.iou_thres,
                                   classes=self.classes, agnostic=self.agnostic_nms, max_det=self.max_det)
        detections = Detections(ims, pred2, None, times=(0,0,0,0), names=self.class_names, shape=(1,3,640,640))
