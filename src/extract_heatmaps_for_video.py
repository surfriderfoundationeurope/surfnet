...
model_weights = (
    "experiments/detection/retrain_3500_images_no_DCNv2/model_114.pth"
)
...
reader = IterableFrameReader(
    video_filename, skip_frames=5, output_shape=(960, 544)
)
...
batched_frames_for_model = torch.stack(
    [transform_for_test()(frame) for frame in frames]
)
...
for frame1, heatmap0, heatmap1, warped_heatmap0 in zip(
    frames[1:], heatmaps[:-1], heatmaps[1:], warped_heatmaps
):
