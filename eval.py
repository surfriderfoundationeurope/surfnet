...
def prec_recall_for_thres(thres, thres_nb, gt, pred, radius):
    ...
    for gt_frame, detection_frame in zip(gt, detections):
        ...
        position_detections = 4*np.argwhere(detection_frame)
...
def compute_model_outputs(arch):
    ...
    transforms = TrainTransforms(600, (600,600), 1, 4)
    ...
    with open(f'predictions_MobileNet_noQ','wb') as f:
...
def load_model_outputs(arch):
    ...
    with open(f'eval_heatmaps_{arch}.pickle','rb') as f:
        all_heatmaps = np.load(f, allow_pickle=True)
    with open(f'eval_gt_{arch}.pickle','rb') as f:
    #with open(f'gt_heatmaps.npy','rb') as f:
        all_gt = np.load(f, allow_pickle=True)

    return all_heatmaps, all_gt 


#def compare_models()
#['res_18','mobilenetv3small','dla_34']
def main(args=None):
    for arch in ['mobilenetv3small']:
