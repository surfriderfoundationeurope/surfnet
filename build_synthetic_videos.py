...
def init_tracker(first_frame, dense_flow_first_frame, object_nb, tractable_band='any'):
    ...
    norm_blocks = np.mean(utils.blockshaped(
        norm, pgcd, pgcd), axis=(1, 2)).reshape(block_size)

    if tractable_band == 'center_left':
        norm_tractacle_region = norm[int(
            3*norm.shape[0]/6):int(4*norm.shape[0]/6), :int(norm.shape[1]/3)]
        norm_blocks_tractacle_regions = norm_blocks[int(
            3*block_size[0]/6):int(4*block_size[0]/6), :int(block_size[1]/3)]
    elif tractable_band == 'lower_half_left':
        norm_tractacle_region = norm[int(
            norm.shape[0]/2):, :int(norm.shape[1]/3)]
        norm_blocks_tractacle_regions = norm_blocks[int(
            block_size[0]/2):, :int(block_size[1]/3)]
    else:
    ...
    dist_to_median = np.abs(
        norm_blocks_tractacle_regions-(object_nb+1)*mean_norm)
    median_block = np.array(np.unravel_index(np.argmin(
        dist_to_median, axis=None), dist_to_median.shape)) + np.array([int(block_size[0]/2), 0])

    roi = pgcd*np.array([median_block[1], median_block[1]+1,
                        median_block[0], median_block[0]+1])

    mask = np.zeros_like(first_frame)
    mask[roi[2]:roi[3], roi[0]:roi[1]] = 255

    p0 = cv2.goodFeaturesToTrack(cv2.cvtColor(
        first_frame, cv2.COLOR_BGR2GRAY), mask=mask[:, :, 0], **flow_tools.feature_params)
...
def condition_tracking(points, init_nb_pts):
    if len(points) != init_nb_pts:
        ...
        if spread_too_high:
            print('Points spreaded too much.')
...
def compute_flow(frame_reader, tractable_band, object_nb):
    ...
    if not ret:
        return [], []
    dense_flow_first_frame = flow_tools.flow_opencv_dense(
        first_frame, second_frame)
    pts_sequence = list()
    p0 = init_tracker(first_frame, dense_flow_first_frame,
                      object_nb, tractable_band)
    if p0 is not None:
    ...
    while(condition_tracking(pts_sequence[-1], init_nb_pts)):
        ...
        pts_sequence.append(new_pts)
        displacement_norm_sequence.append(
            utils.displacement_norm(pts_sequence))
...
def compute_flows(frame_reader, tractable_band, nb_objects):
    ...
    for object_nb in range(nb_objects):
        pts_sequence, displacement_norm_sequence = compute_flow(
            frame_reader, tractable_band, object_nb=object_nb)
...
def add_trash_objects(frame_reader, tractable_band, nb_objects, output_original_shape=False):
    ...
    if output_original_shape:
        ...
        init_area = (frame_reader.original_width *
                     frame_reader.original_height) / 500
        for i, pts_sequence in enumerate(usable_pts_sequences):
            usable_pts_sequences[i] = [
                frame_reader.init_rescale_factor * pts for pts in pts_sequence]
    else:
        init_area = (frame_reader.new_shape[0]
                     * frame_reader.new_shape[1]) / 500
    ...
    for object_nb in range(len(usable_pts_sequences)):
        ...
        # print(trash_area)
        init_shapes.append(utils.rescaling(
            init_coeff_reshape, trash_img.shape))
    ...
    for frame_nb in range(shortest_sequence_length):
        ...
        for object_nb in range(len(usable_pts_sequences)):
            ...
            if np.any(np.isnan(displacement_norm_sequence)):
                break
            shape = utils.rescaling(displacement_norm_sequence[min(
                frame_nb, len(displacement_norm_sequence)-1)], init_shape)
            center, (frame, bbox) = utils.overlay_trash(
                frame, trash_img, alpha, pts_sequence[frame_nb], shape)
...
def main(args):
    ...
    video_filenames = [
        vid_name for vid_name in vid_names if '.MP4' in vid_name]
    ...
    for video_nb, video_filename in enumerate(video_filenames):
        ...
        if not output_original_shape:
            shape = frame_reader.new_shape
        ...
        while total_frames_read < (total_num_frames - 100):
            ...
            if frame_reader.original_shape_mode:
                frame_reader.set_original_shape_mode(False)
            total_frames_read += nb_frames_read + \
                int(total_num_frames / nb_extracts_per_vid)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--original-res', dest='original_res', action='store_true')
    parser.add_argument('--rescale', type=int, default=3)
    parser.add_argument('--nb-extracts-per-vid',
                        dest='nb_extracts_per_vid', type=int, default=20)
    parser.add_argument('--synthetic-objects', type=str)
    parser.add_argument('--nb-frames-without-object', type=int,
