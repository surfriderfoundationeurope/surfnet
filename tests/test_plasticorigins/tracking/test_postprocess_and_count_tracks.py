...
def test_filter_tracks():
    ...
    assert type(filtered_results[0][2]) == float or type(filtered_results[0][2]) == np.float64
    assert type(filtered_results[0][3]) == float or type(filtered_results[0][2]) == np.float64
    assert type(filtered_results[0][4]) == float or type(filtered_results[0][2]) == np.float64
...
def test_post_process_for_api():
    ...
    if len(resp_data['detected_trash']) > 0:
        assert set(resp_data['detected_trash'][0].keys()) == set(['label', 'id', 'frame_to_box', 'avg_conf'])
