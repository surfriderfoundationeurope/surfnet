...
from plasticorigins.tracking.utils import (
    get_detections_for_video, write_tracking_results_to_file, GaussianMixture,
    exp_and_normalise, gather_filenames_for_video_in_annotations, read_tracking_results,
    overlay_transparent
    )
...
def test_gather_filename_for_video_annotations():
    ...
    images = [
        {"frame_id": i,
         "video_id": 1,
         "file_name": f"1_{i}_.mp4"}
        for i in range(10)
    ]
    images.extend([
        {"frame_id": i,
         "video_id": 2,
         "file_name": f"2_{i}_.mp4"}
        for i in range(10)
    ])
    video = {
        'id' : 1,
...
def test_overlay_transparent():
    ...
    background_1 = overlay_transparent(background=background.copy(),
                                      overlay=overlay,
                                      x=100,
                                      y=100)
    
    background_2 = overlay_transparent(background=background.copy(),
                                      overlay=overlay,
                                      x=600,
                                      y=100)
    
    background_3 = overlay_transparent(background=background.copy(),
                                      overlay=overlay,
                                      x=450,
                                      y=450)
    
    background_4 = overlay_transparent(background=background.copy(),
                                      overlay=overlay_1,
                                      x=100,
                                      y=100)
