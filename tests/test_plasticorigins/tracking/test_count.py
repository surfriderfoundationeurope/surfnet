...
video_file_path = "tests/ressources/validation_videos/T1_trim.mp4"
...
def test_evaluate_detected_count():
    ...
    n_det, _ = count_detected_objects(results, video_file_path, compare=False)
    assert n_det == 1


def test_video_count_truth():
    n = video_count_truth(video_file_path)
    assert n == 2
