...
script_path = os.path.join(functions_dir, "create_n_trash.py")
...
result_dataset_path = os.path.join(
    current_dir, "../../ressources/artificial_data_n_objects")
...
def test_label_components():
    ...
    # Run the shell command and capture the output
    output = subprocess.check_output(cmd, shell=True)
    files = os.listdir(result_labels_path)
    files = sorted(files)
    print(files)
    for i in range(len(files)):
        file_path = os.path.join(result_labels_path, files[i])
